...
import com.wxl.common.utils.Reflections;
 ...
 *
 */
public class ImportExcel<T extends BaseEntity> {
		...
		try {
			...
			if (cell != null) {
				if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
					// if (HSSFDateUtil.isCellDateFormatted(cell)) { // Excel
					// Date类型处理
					// val =
	 ...
	 *            导入分组
	 */
	public List<T> getDataList(T cls, int... groups) throws InstantiationException, IllegalAccessException {
...
//						val = DictUtils.getDictValue(val.toString(), ef.dictType(), "");
					...
					try {
						if (valType == String.class) {
						...
						} else if (valType == Integer.class || valType.getSimpleName() == "int") {
							if (val != null && StringUtils.isNoneBlank(val.toString())) {
								val = Double.valueOf(val.toString()).intValue();
							} else {
								val = 0;
							}
						} else if (valType == Long.class || valType.getSimpleName() == "long") {
							if (val != null && StringUtils.isNoneBlank(val.toString())) {
								val = Double.valueOf(val.toString()).longValue();
							} else {
								val = 0;
							}
						} else if (valType == Double.class || valType.getSimpleName() == "double") {
							if (val != null && StringUtils.isNoneBlank(val.toString())) {
								val = Double.valueOf(val.toString());
							} else {
								val = 0.0;
							}
						} else if (valType == Float.class || valType.getSimpleName() == "float") {
							if (val != null && StringUtils.isNoneBlank(val.toString())) {
							...
							if (val != null && StringUtils.isNoneBlank(val.toString())) {
								...
								// ef.format() );
								// val = (Date)sdf.parse(val.toString());
								if (val instanceof String) {
									val = DateUtil.getJavaDate(Double.valueOf((String) val));
								} else {
...
//						ex.printStackTrace();
	...
	public List<Object> getColDatas(int k) {

		List<Object> datas = new ArrayList<Object>();
		for (int i = this.getDataRowNum(); i < this.getLastDataRowNum(); i++) {
			Row row = this.getRow(i);
			Object val = this.getCellValue(row, k);
			datas.add(val);
		}
		return datas;
	}
