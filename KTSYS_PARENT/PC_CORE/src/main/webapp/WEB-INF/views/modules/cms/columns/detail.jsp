			...
			<div class="layui-card-body">
				<form class="layui-form layui-form-pane" action="#" lay-filter="component-form-group">
					...
					<div class="layui-form-item">
						...
						<div class="layui-input-block">
							<input id="name" name="name" value="${columns.name}" lay-verify="required" placeholder="请输入" autocomplete="off" class="layui-input" type="text" readonly>
						</div>
					</div>
					<div class="layui-form-item">
					...
					<div class="layui-form-item">
						...
						<div class="layui-input-block">
							<input type="checkbox" name="firstnav" value="${columns.firstnav}" lay-skin="switch" <c:if test="${columns.firstnav eq 'on'}">checked</c:if> lay-text="是|不是" disabled>
						</div>
					</div>
					<div class="layui-form-item">
						<label class="layui-form-label">打开目标：</label>
						<div class="layui-input-block">
							<input type="radio" name="target" value="_blank" <c:if test="${columns.target eq '_blank'}">checked</c:if> title="在新窗口中打开" disabled>
							<input type="radio" name="target" value="_self" <c:if test="${columns.target eq '_self'}">checked</c:if> title="在相同的框架中打开" disabled>
							<input type="radio" name="target" value="_parent" <c:if test="${columns.target eq '_parent'}">checked</c:if> title="在父框架集中打开" disabled>
							<input type="radio" name="target" value="_top" <c:if test="${columns.target eq '_top'}">checked</c:if> title="在整个窗口中打开" disabled>
						</div>
					</div>
					<div class="layui-form-item">
						<label class="layui-form-label">连接：</label>
						<div class="layui-input-block">
							<input type="text" name="linkAddress" value="${columns.linkAddress}" placeholder="请输入连接地址" autocomplete="off" class="layui-input" readonly>	
						</div>
					</div>
					<div class="layui-form-item">
