...
layui.define(["element","jquery"],function(exports){
            ...
            if(data[i].childrens != undefined && data[i].childrens.length > 0){
                ...
                for(var j=0;j<data[i].childrens.length;j++){
                    ...
                    if(data[i].childrens[j].target == "_blank"){
                        ulHtml += '<dd><a data-url="'+data[i].childrens[j].href+'" target="'+data[i].childrens[j].target+'">';
                    }else{
                        ulHtml += '<dd><a data-url="'+data[i].childrens[j].href+'">';
                    }
                    if(data[i].childrens[j].icon != undefined && data[i].childrens[j].icon != ''){
                        // if(data[i].childrens[j].icon.indexOf("icon-") != -1){
                        //     ulHtml += '<i class="seraph '+data[i].childrens[j].icon+'" data-icon="'+data[i].childrens[j].icon+'"></i>';
                        // }else{
                        //     ulHtml += '<i class="layui-icon" data-icon="'+data[i].childrens[j].icon+'">'+data[i].childrens[j].icon+'</i>';
						// }
						// ulHtml += '<i class="fa fa-' + data[i].childrens[j].icon + '"></i>';

						ulHtml += data[i].childrens[j].icon;
                    }
                    ulHtml += '<cite>'+data[i].childrens[j].name+'</cite></a></dd>';
                }
                ulHtml += "</dl>";
            }else{
	...
	Tab.prototype.tabAdd = function(_this){
		...
		if(_this.attr("target") == "_blank"){
			window.open(_this.attr(ctx + "data-url"));
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				element.tabAdd(tabFilter, {
			        title : title,
			        content :"<iframe src='" + ctx + _this.attr("data-url") + "' data-id='"+tabIdIndex+"'></frame>",
			        id : new Date().getTime()
			    })
				//当前窗口内容
				var curmenu = {
